...
async function iniciarCancelamento(sockIgnorado, jid, senderNumber, stateMachine, STEPS) {
    ...
    if (agendamentos.length === 0) {
        await sendDelayedText(null, jid, 'Não tens agendamentos futuros para cancelar no momento.');
    ...
    await sendInteractiveMenu(null, jid, '🗑️ *Cancelar Agendamento*\nQual horário pretendes cancelar?', opcoes);
}
...
async function processarCancelamento(sockIgnorado, jid, textMessage, senderNumber, stateMachine, STEPS) {
    ...
    if (textMessage.trim() === '0' || escolha === 0) {
        stateMachine.set(senderNumber, { step: STEPS.MENU_PRINCIPAL, data: {} });
        await sendDelayedText(null, jid, 'Cancelamento abortado. A voltar ao menu principal...');
        return;
    }

    const agendamentoAlvo = userState.data.agendamentos[escolha - 1];
    if (!agendamentoAlvo) {
        await sendDelayedText(null, jid, 'Opção inválida. Por favor, seleciona o número ou opção correspondente na lista.');
        return;
    }
